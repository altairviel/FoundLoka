      ...
      >
        ...
        <div className="ff-nav-links">
          {navLinks.map((l) => (
            <button
              key={l.path}
              className={`ff-nav-link${location.pathname === l.path ? ' active' : ''}`}
              ...
              style={{
                ...
                cursor: 'pointer',
                padding: '8px 12px',
                color: location.pathname === l.path ? T.green : T.gray700,
              ...
              <div style={{ position: 'absolute', right: -40, top: '120%', zIndex: 200 }}>
                ...
                >
                  ...
                  >
                    ...
                    {unreadCount > 0 && (
                      <button onClick={handleMarkAllRead} style={{ fontSize: 12, color: T.green, background: 'none', border: 'none', cursor: 'pointer', fontWeight: 500 }}>
                        Tandai semua dibaca
                      </button>
                    )}
                  </div>
                  ...
                  {loadingNotif ? (
                    <div style={{ padding: '2rem', textAlign: 'center', color: T.gray500, fontSize: 13 }}>Memuat...</div>
                  ) : notifications.length === 0 ? (
                    <div style={{ padding: '2rem', textAlign: 'center', color: T.gray500, fontSize: 13 }}>Belum ada notifikasi.</div>
                  ) : (
                    notifications.map((n) => (
                      <div
                        key={n.id}
                      ...
                      >
                        <div style={{ fontSize: 13, fontWeight: n.is_read ? 400 : 600, color: T.gray900, marginBottom: 2 }}>{n.message || n.title}</div>
                        <div style={{ fontSize: 11, color: T.gray500 }}>
                          ...
                          {!n.is_read && <span style={{ marginLeft: 8, color: T.green, fontWeight: 600 }}>● Baru</span>}
                        </div>
                      </div>
                    ))
                  )}
          ...
          <div style={{ position: 'relative' }}>
            ...
            >
              ...
              <div className="ff-user-info" style={{ textAlign: 'left' }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: T.gray900, lineHeight: 1.2 }}>{user?.name?.split(' ')[0] || 'Pengguna'}</div>
                <div style={{ fontSize: 11, color: T.gray500, lineHeight: 1 }}>{role === 'investor' ? 'Investor' : role === 'owner' ? 'Pemilik UMKM' : 'Admin'}</div>
              </div>
          ...
          {navLinks.map((l) => (
            <button
              ...
              style={{
                ...
                color: location.pathname === l.path ? T.green : T.gray700,
                fontWeight: location.pathname === l.path ? 600 : 500,
                background: location.pathname === l.path ? T.greenLight : 'transparent',
              }}
            >
              {l.label}
            </button>
