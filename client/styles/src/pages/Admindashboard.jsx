    ...
    <div style={{
      ...
      <div style={{ background: T.white, borderRadius: 12, padding: '2rem', maxWidth: 360, width: '90%', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button className="ff-btn ff-btn-sm" onClick={onCancel}>Batal</button>
          <button
            className="ff-btn ff-btn-sm"
            style={{ background: '#DC2626', color: T.white, borderColor: '#DC2626' }}
            onClick={onConfirm}
          >
            Ya, Lanjutkan
          </button>
  ...
  return (
    <tr>
      {Array.from({ length: cols }).map((_, i) => (
        <td key={i}><div style={{ height: 14, background: T.gray100, borderRadius: 4, width: '70%' }} /></td>
      ))}
    </tr>
  );
}
  ...
  return (
    <>
      ...
      <div className="ff-grid-4" style={{ marginBottom: '1.5rem' }}>
        ...
        <StatCard label="Campaign Aktif"        value={s.active_campaigns  ?? '—'} accent />
        <StatCard label="Campaign Menunggu"     value={s.pending_campaigns ?? '—'} />
        <StatCard label="Total Dana Terkumpul"  value={s.total_funded != null ? fmt(s.total_funded) : '—'} accent />
      </div>
      ...
      <div className="ff-grid-2">
            ...
            return (
              <div key={item.label} style={{ marginBottom: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4 }}>
                  ...
                  <span style={{ fontWeight: 600, color: T.gray900 }}>{item.value} ({pct}%)</span>
                </div>
        ...
        <div className="ff-card">
          ...
          {[
            { label: 'Total Investasi',   value: s.total_investments != null ? fmt(s.total_investments) : '—' },
            { label: 'Total Penarikan',   value: s.total_withdrawals != null ? fmt(s.total_withdrawals) : '—' },
            { label: 'Campaign Selesai',  value: s.completed_campaigns ?? '—' },
            { label: 'Campaign Ditolak',  value: s.rejected_campaigns  ?? '—' },
          ].map((item) => (
...
function CampaignsTab({ onToast }) {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      onToast(
        action === 'approve'  ? 'Campaign disetujui.'       :
        action === 'reject'   ? 'Campaign ditolak.'         :
                                'Dana campaign dicairkan.',
        'success'
      );
                  ...
                  : filtered.map((c) => {
                    ...
                    return (
                      <tr key={c.id}>
                        ...
                        <td style={{ color: T.green, fontWeight: 600 }}>
                          {fmt(parseFloat(c.collected_amount || c.raised || 0))}
                        </td>
...
export default function AdminDashboard({ user }) {
  ...
  return (
    <div style={{
      display: 'flex', minHeight: 'calc(100vh - 56px)', position: 'relative',
      ...
      {isMobile && (
        <button
          ...
          style={{
            ...
            background: '#7C3AED', color: T.white, border: 'none',
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <div style={{
            ...
            fontSize: 14, fontWeight: 700, color: '#7C3AED', marginBottom: '0.5rem',
          }}>
            {initials}
          </div>
