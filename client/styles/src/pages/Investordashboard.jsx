...
import Sidebar from '../styles/src/components/Sidebar';
            ...
            <tbody>
              {portfolio.map((p) => (
                <tr key={p.campaign}>
                  ...
                  <td>
                    <span style={{ color: T.green, fontWeight: 500 }}>{p.return}</span>
                  </td>
                  <td>
                    <span className={`ff-badge ${p.status === 'Aktif' ? 'ff-badge-green' : 'ff-badge-gray'}`}>{p.status}</span>
                  </td>
          ...
          <tbody>
            {transactions.map((t, i) => (
              <tr key={i}>
                ...
                <td>
                  <span className="ff-badge ff-badge-green">{t.status}</span>
                </td>
              </tr>
            ))}
          </tbody>
...
export default function InvestorDashboard() {
  ...
  const saldoFooter = (
    <div style={{ padding: 12, background: T.greenLight, borderRadius: 8 }}>
      ...
      <button className="ff-btn ff-btn-sm" style={{ marginTop: 8, width: '100%', fontSize: 12 }}>
        Top Up
      </button>
    </div>
  );
