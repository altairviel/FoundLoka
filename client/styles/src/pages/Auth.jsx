...
export default function Auth({ setRole, setPage, setUser }) {
  ...
  // ✅ UPDATE: Menambahkan field phone pada state registerForm
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    // ✅ UPDATE: Ekstrak phone dari registerForm
    ...
    try {
      // 1️⃣ Hit POST /api/auth/register dengan phone payload
      const { data } = await api.post('/auth/register', {
        name,
        email,
        phone, // ✅ UPDATE: Dikirimkan ke back-end
        password,
        ...
        <div style={{ maxWidth: 400, width: '100%' }}>
          ...
          {isRegister && (
            <form onSubmit={handleRegister}>
              ...
              {/* ✅ BARU: Input Field untuk No. HP / Phone */}
              <div style={{ marginBottom: '1rem' }}>
              ...
              <div style={{ marginBottom: '2rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '0.5rem' }}>
                  {[
                    ...
                    <div key={key} onClick={() => setSelectedRole(key)} style={{ border: `2px solid ${selectedRole === key ? T.green : T.gray200}`, background: selectedRole === key ? T.greenLight : T.white, padding: '1rem', borderRadius: 8, cursor: 'pointer', textAlign: 'center', transition: 'all 0.2s' }}>
                      <div style={{ fontSize: 24, marginBottom: 4 }}>{emoji}</div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: selectedRole === key ? T.greenDark : T.gray700 }}>{label}</div>
                      <div style={{ fontSize: 11, color: T.gray500, marginTop: 2 }}>{sub}</div>
                    </div>
                  ))}
                </div>
              </div>
