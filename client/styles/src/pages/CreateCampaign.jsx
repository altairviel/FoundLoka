...
export default function CreateCampaign({ user, onSuccess, onCancel }) {
  ...
  const monthlyAmt   = form.repayment_type === 'cicilan' && tenorNum > 0
    ? Math.round(totalReturn / tenorNum) : 0;
    ...
    <div style={{ background: T.gray50, minHeight: 'calc(100vh - 56px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div className="ff-card" style={{ maxWidth: 480, width: '100%', textAlign: 'center', padding: '3rem 2rem' }}>
        ...
        <p style={{ fontSize: 14, color: T.gray500, lineHeight: 1.7, marginBottom: '2rem' }}>
          Kampanye <strong>{form.title}</strong> sedang menunggu persetujuan admin.
          Proses verifikasi biasanya 1–3 hari kerja. Kamu akan mendapat notifikasi saat disetujui.
        </p>
        <div style={{ background: T.greenLight, borderRadius: 8, padding: '12px', marginBottom: '1.5rem', fontSize: 13 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ fontWeight: 600 }}>
              {form.repayment_type === 'cicilan' ? `Cicilan ${tenorNum} bulan` : 'Lunas di akhir'}
            </span>
          </div>
        </div>
    ...
    <div style={{ background: T.gray50, minHeight: 'calc(100vh - 56px)', padding: '2rem 0' }}>
      <div className="ff-container" style={{ maxWidth: 700 }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          <button className="ff-btn ff-btn-sm" onClick={onCancel} style={{ marginBottom: '1rem' }}>← Kembali</button>
        ...
        <div className="ff-card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div>
                ...
                <input style={inputStyle} type="text" value={form.title}
                  onChange={set('title')} placeholder="Contoh: Warung Makan Bu Siti" />
              </div>
              ...
              <div>
                ...
                <div style={{
                  ...
                  fontSize: 12, color: form.description.length < 50 ? '#D97706' : T.green, marginTop: 4,
                }}>
          ...
          {step === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div>
                ...
                {form.target_amount && !isNaN(form.target_amount) && (
                  <div style={{ fontSize: 12, color: T.green, marginTop: 4 }}>
                    = {rupiah(parseFloat(form.target_amount))}
                  </div>
                )}
              </div>
              ...
              {/* Return rate */}
              <div>
                <label style={labelStyle}>Return Rate (% per tahun) <span style={{ color: '#EF4444' }}>*</span></label>
                ...
                <div style={{ fontSize: 12, color: T.gray500, marginTop: 4 }}>
                  Berapa % dari modal yang akan kamu kembalikan di atas pokok. Range: 1% – 50%
                </div>
              </div>

              {/* ── JENIS PENGEMBALIAN ── */}
              <div>
                <label style={labelStyle}>Jenis Pengembalian <span style={{ color: '#EF4444' }}>*</span></label>
                  ...
                  {[
                    ...
                    return (
                      <button
                        key={opt.val}
                        type="button"
                        onClick={() => { setForm((f) => ({ ...f, repayment_type: opt.val })); setError(''); }}
                        style={{
              ...
              {/* Deadline */}
              <div>
                <label style={labelStyle}>Deadline Pendanaan <span style={{ color: T.gray500, fontWeight: 400 }}>(opsional)</span></label>
                <input style={inputStyle} type="date"
                  min={new Date().toISOString().split('T')[0]}
                  value={form.deadline} onChange={set('deadline')} />
                <div style={{ fontSize: 12, color: T.gray500, marginTop: 4 }}>
                  Batas waktu kampanye menerima investasi dari investor.
                </div>
              </div>
              ...
              {targetNum > 0 && rateNum > 0 && (
                <div style={{
                  ...
                  <div style={{ fontWeight: 600, color: T.greenDark, marginBottom: 12, fontSize: 14 }}>
                    📊 Preview Kewajiban ke Investor
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {[
                      ...
                      form.repayment_type === 'cicilan' && tenorNum > 0
                        ? [`Cicilan per bulan (${tenorNum} bln)`, rupiah(monthlyAmt)]
                        : ['Dibayar', 'Sekaligus di akhir tenor'],
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Contoh: Jl. Sudirman No. 12, Kel. Sukamaju, Kec. Tampan, Pekanbaru, Riau 28291"
                />
              </div>
          ...
          {step === 3 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <p style={{ fontSize: 14, color: T.gray500, marginBottom: 4 }}>
                Periksa kembali sebelum diajukan ke admin untuk diverifikasi.
              </p>
              ...
              <div style={{ background: T.gray50, border: T.border, borderRadius: 8, padding: '1rem' }}>
                ...
                {[
                  ...
                  form.repayment_type === 'cicilan'
                    ? ['Cicilan per bulan', rupiah(monthlyAmt)]
                    : ['Bayar sekaligus', rupiah(totalReturn)],
