...
import { campaigns, umkmTransactions } from "../styles/dummyData";
    ...
    <div style={{ display: "flex", minHeight: "calc(100vh - 56px)" }}>
      ...
      <aside className="ff-sidebar-wrapper" style={{ width: 240, borderRight: T.border, padding: "1.5rem 1rem", background: T.white }}>
        <div style={{ marginBottom: "2rem" }}>
          ...
          <span className="ff-badge ff-badge-green" style={{ marginTop: 6 }}>{campaign.sector}</span>
        </div>
        {SIDEBAR_LINKS.map((l) => (
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {MILESTONES.map((m, i) => (
              <div key={m.label} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: i < MILESTONES.length - 1 ? T.border : "none" }}>
                ...
                <span style={{ fontSize: 14, color: m.done ? T.gray900 : T.gray500, fontWeight: m.done ? 500 : 400 }}>
                  {m.label}
                </span>
              </div>
            ))}
                ...
                {umkmTransactions.map((t, i) => (
                  <tr key={i}>
                    ...
                    <td style={{ fontWeight: 600, color: t.status === "Masuk" ? T.green : T.gray700 }}>
                      {t.status === "Masuk" ? "+" : "−"}{fmt(t.amount)}
                    </td>
                    <td>
                      <span className={`ff-badge ${t.status === "Masuk" ? "ff-badge-green" : "ff-badge-amber"}`}>
                        {t.status}
                      </span>
                    </td>
                  </tr>
